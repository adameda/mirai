  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "40px 48px", background: T.bg, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ background: T.white, borderRadius: 22, border: `1px solid ${T.border}`, overflow: "hidden", boxShadow: "0 2px 12px rgba(15,31,61,0.05)" }}>
        ...
        {students.length === 0 && (
          <div style={{ padding: "28px 20px", textAlign: "center", fontSize: 13, color: T.muted }}>
            Aucun élève dans la classe pour l'instant.
          </div>
        )}
        {students.map((s, i) => (
          <div key={s.id} style={{ display: "grid", gridTemplateColumns: "minmax(120px,1.15fr) 0.7fr 0.7fr 0.7fr 0.7fr", gap: 12, padding: "14px 20px", borderBottom: i < students.length - 1 ? `1px solid ${T.border}` : "none", alignItems: "center", background: i % 2 === 0 ? T.white : T.bg }}>
