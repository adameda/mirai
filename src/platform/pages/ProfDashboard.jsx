...
export default function ProfDashboard() {
  ...
  const jalonsRows = [
    ...
    { title: `Sauvegarder ${config.obj2.target} domaines`, periode: `Date limite : ${config.obj2.date ?? "—"}`, prog: Math.round((nbDomainesOk / n) * 100) },
    { title: `Sauvegarder ${config.obj3.target} formations`, periode: `Date limite : ${config.obj3.date ?? "—"}`, prog: Math.round((nbFormOk / n) * 100) },
    { title: `Sauvegarder ${config.obj4.target} métiers`, periode: `Date limite : ${config.obj4.date ?? "—"}`, prog: Math.round((nbMetiersOk / n) * 100) },
  ];
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: "40px 48px", background: T.bg, fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 22 }}>
        {[
          ...
          { label: `Objectif métiers (${config.obj4.target} fav.)`, value: nbMetiersOk, total: n, sym: "◈" },
        ].map((s) => (
